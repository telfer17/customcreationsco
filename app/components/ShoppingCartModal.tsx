        ...
        <div className='flex flex-col justify-between h-full'>
          <div className='flex-1 mt-8 overflow-y-auto'>
            <ul className='-my-6 divide-y divide-gray-200'>
              {cartCount === 0 ? (
                <h1 className='py-6'>Shopping cart is empty.</h1>
              ) : (
                <>
                  {Object.values(cartDetails ?? {}).map((entry) => (
                    <li key={entry.id} className='flex py-6 overflow-hidden border border-gray-200 rounded-md'>
          ...
          <div className='px-4 py-6 border-t border-gray-200 sm:px-6'>
            ...
            <div className='flex justify-center mt-4 mb-4 text-sm text-center text-gray-500'>
              <Button className='w-full' variant={'secondary'} onClick={() => handleCartClick()}>
                Continue Shopping
              </Button>
            </div>
          </div>
